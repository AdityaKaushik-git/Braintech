...
export default function Contact() {
  ...
  const contactItems = [
    ...(!academy.phone.includes('XXXXXXXXXX') ? [{
      ...
      hoverClass: 'group-hover:text-cyan-400', waBg: false,
    }] : []),
    ...
    }] : []),
    {
      ...
      value: 'Y11, 30 Feet Rd, Pocket A\nBudh Vihar Phase I, New Delhi – 110086',
      hoverClass: '', waBg: false,
    },
  ];

  return (
    <section id="contact" className="section-padding bg-navy relative overflow-hidden" aria-labelledby="contact-heading">
      <div className="absolute inset-0 grid-pattern" aria-hidden="true" />
      <div className="absolute top-0 left-0 w-96 h-96 bg-blue-600/10 rounded-full blur-3xl" aria-hidden="true" />
      <div className="absolute bottom-0 right-0 w-72 h-72 bg-cyan-400/8 rounded-full blur-3xl" aria-hidden="true" />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-14 items-start">
          ...
          <ScrollReveal animation="fade-right" duration={700}>
            <div>
              <p className="text-xs font-semibold tracking-widest uppercase text-cyan-400 mb-3">Get In Touch</p>
              <h2 id="contact-heading" className="section-title-light mb-5">Enquire About Admission</h2>
                ...
                {contactItems.map((item, i) => {
                  ...
                  return (
                    <El key={i} {...props} className="flex items-center gap-4 group">
                      <div className={`w-12 h-12 rounded-xl border border-white/15 flex items-center justify-center flex-shrink-0 transition-colors ${item.waBg ? 'bg-white/8 group-hover:bg-[#25D366]/20' : 'bg-white/8 group-hover:bg-white/15'}`}>
                        {item.waBg ? (
                        ...
                        ) : Icon ? <Icon size={20} className="text-cyan-400" /> : null}
                      </div>
                      <div>
                        <div className="text-xs text-slate-400 mb-0.5">{item.label}</div>
                        <div className={`text-white font-semibold text-sm transition-colors whitespace-pre-line ${item.hoverClass}`}>
                          {item.value}
                        </div>
                      </div>
                    </El>
